...
const VendorProfile = () => {
  ...
  const loadDocuments = async (page = 1) => {
    try {
      setLoading(true);
      const response = await fetchVendorDocuments(page, itemsPerPage);
  ...
  const handleApprove = async (id) => {
    try {
      await approveVendorDocument({ id: parseInt(id), is_approved: true });
  ...
  const handleDisapprove = async (id) => {
    try {
      await approveVendorDocument({ id: parseInt(id), is_approved: false });
      await loadDocuments(currentPage); // reload current page
    } catch (err) {

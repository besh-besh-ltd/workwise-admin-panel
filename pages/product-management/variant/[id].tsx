...
import { searchAllVariants, mapVariantWithVendor, getVariantSpecifications } from '@/utils/services/product-management';
...
const VariantView: React.FC = () => {
  ...
  const [currentVendors, setCurrentVendors] = useState<unknown[]>([]);
  ...
  const customStyles = {
    option: (provided: React.CSSProperties, state: { isSelected: boolean }) => ({
      ...provided,
  ...
  const getVendors = (): void => {
    vendorList(null)
      .then((rsp: { data: Array<{ id: string | number; organization_name?: string; name: string; email?: string; mobile?: string }> }) => {
  ...
  const handleMapVendor = async (): Promise<void> => {
    if (!selectedVendor) {
    ...
    try {
      ...
      toast.error((error as Error).message || "Failed to map vendor");
      setLoading(false);
    }
  };

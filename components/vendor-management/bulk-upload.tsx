...
type TabType = 'summary' | 'vendors' | 'rows' | 'errors';

const BulkVendorUpload: React.FC = () => {
  ...
  const handleFileSelect = (e: ChangeEvent<HTMLInputElement> | { target: { files: File[] } }): void => {
    ...
    try {
      ...
      if ((response as any).status === 1) {
        ...
        const successMsg = summary
          ...
          : ((response as any).message || 'Upload completed successfully');
        ...
        const errorMsg = summary
          ? `Upload failed! ${summary.vendorsFailed || 0} vendors failed, ${summary.productsFailed || 0} products failed. ${(response as any).message || ''}`
          : ((response as any).message || 'Upload failed');
  ...
  const downloadReport = (): void => {
    ...
    const escapeHtml = (str: string | number | undefined | null): string => (str || '-').toString().replace(/</g, '&lt;').replace(/>/g, '&gt;');
            ...
            <div className="card">
              ...
              <div className="card-body">
                ...
                <div className="tab-content mt-3">
                          ...
                          <tbody>
                            {results.vendors?.map((vendor) => (
                              <tr key={vendor.vendorId}>
                                ...
                                <td>
                                  <span className={`badge badge-${vendor.status === 'created' ? 'success' :
                          ...
                          <tbody>
                            {results.rows?.map((row, index) => (
                              <tr key={`${row.vendorId}-${index}`}>
                                <td>{row.excelRow}</td>
                                <td>
                                  <span className={`badge badge-${row.type === 'vendor' ? 'primary' :
                                ...
                                <td>
                                  <span className={`badge badge-${row.status === 'success' || row.status === 'created' ? 'success' :
                  ...
                  {/* Errors Tab */}
                  {activeTab === 'errors' && results.errors && results.errors.length > 0 && (
                    <div className="tab-pane active">
                          ...
                          <table className="table table-sm table-bordered mb-0">
                            ...
                            <tbody>
                              {results.errors.map((error, index) => (
                                <tr key={`${error.row}-${error.vendor}-${index}`}>
                                  <td>{error.row || '-'}</td>
                                  <td>{error.vendor || '-'}</td>
                                  <td>{error.error}</td>
                                </tr>

...
const SpocAddModal = ({ openModal, closeModal, handleAddSpoc, countryCode }) => {
  ...
  const validateBody = async () => {
    try {
      ...
      // Fixed: Ensure we don't add extra + signs
      // If country_code already has a +, use it as is, otherwise add one
      const countryCodeFormatted = spocDetails.country_code || "";
      
      // Merge country_code + spoc_mobile before sending
      const formattedMobile = `${countryCodeFormatted}-${spocDetails.spoc_mobile.replace(/^[0\s]+/, '')}`;

      // Remove country_code from final payload
            ...
            <Col md={6} className="mb-2">
              <Form.Group controlId="spoc_mobile">
                ...
                <div className="d-flex">
                  {/* Country Code Dropdown - Fixed to not include + in value */}
                  <Form.Select
                    name="country_code"

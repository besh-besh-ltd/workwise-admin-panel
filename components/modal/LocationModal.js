...
const LocationModal = ({ 
 ...
 useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    const countryName = countryList.find(c => c.id == selectedCountry)?.country_name || "";
    const stateName = states.find(s => s.id == selectedState)?.state_name || "";
    const cityName = cities.find(c => c.id == selectedCity)?.city_name || "";
    ...
    try {
      ...
      // await saveLocationAPI(location);
      onSave(location);
      handleClose();
    } catch (error) {

...
interface VendorVariantMappingModalProps {
  isVisible: boolean;
  onCancel: () => void;
...
const VendorVariantMappingModal: React.FC<VendorVariantMappingModalProps> = ({ isVisible, onCancel, vendor, onSuccess }) => {
  ...
  const [approvedBy, setApprovedBy] = useState<ApprovedOption[]>([]);
  ...
  useEffect(() => {
    ...
    vendorApproveList()
      .then((res : any) => {
        const opts = (res?.data || []).map((s: any) => ({ label: s.vendor_approve, value: s.id }));
  ...
  const doSearch = useCallback(async (term: string) => {
    ...
    setSearching(true);
    try {
      const res : any = await searchAllVariants(null, term, null, null, null, null, null, null, 1, 10);
      const list = res?.data || [];
      const formatted: Variant[] = list.map((v: any) => ({
        id: v.id,
  ...
  const submitQueue = async () => {
    ...
    setSubmitting(true);
    try {
      const resp : any = await bulkMapVariantWithVendor(queue);
          ...
          <div className="modal-body p-3" style={{ maxHeight: 600, overflow: 'auto' }}>
            ...
            <div className="form-row">
              <div className="form-group col-md-7">
                ...
                <Select isMulti options={approvedOptions} value={approvedBy} onChange={(val) => setApprovedBy(val as ApprovedOption[])} classNamePrefix="select" placeholder="Select approvals" />
              </div>

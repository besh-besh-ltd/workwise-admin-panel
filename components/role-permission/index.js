...
function RolesPermission() {
    ...
    const [queryUserDetails, setQueryUserDetails] = useState(null);
    ...
    const handleDataMemberChange = (selectedOption) => {
    ...
    const handleRolePermission = () => {
        ...
        if (selectedIds?.length === 0) {
            return toast.error("Please select at least 1 menu");
        }
    ...
    const getSelectedUserRolesDetails = () => {
        ...
        getRolesDetails(selectedId)
            .then((res) => {
                const arr = res?.data?.map((item) => item?.menu_id);
                setSelectedIds(arr);
            })
            .catch((err) => {

...
interface RFQDetail {
    ...
    ra_start_date: string;
    ra_end_date: string;
    admin_service_details: AdminServiceDetail[] | null;
...
interface ProductWiseData {
    ...
    product_description: string;
    specs: ProductSpec[];
    vendors: ProductWiseVendor[];
}

interface ProductStats {
    totalVendors: number;
    respondedVendors: number;
    regrettedVendors: number;
    pendingVendors: number;
    privateVendors: number;
...
const RFQDetails: React.FC = () => {
    ...
    const [rfqDetails, setRfqDetails] = useState<RFQDetail | null>(null);
    ...
    const updateAdminStatus = ({ status, comment }: StatusPayload): void => {
        let payload = {
            rfq_id: parseInt(rfq_id as string),
            status,
            comment
        }
        setLoading(true);
    ...
    const getRfqById = (): void => {
        setLoading(true);
        getRFQDetails(rfq_id as string)
            .then((res : any) => {
    ...
    const handleSendSelectiveReminder = async (vendorIds: number[], useMailGun: boolean): Promise<void> => {
        try {
            const response : any = await sendSelectiveReminder(rfq_id as string, vendorIds, useMailGun);
    ...
    const isAllProductsFinalized = (vendorDetails: VendorDetail[] | null): boolean => {
    ...
    const productWiseData = useMemo((): ProductWiseData[] => {
        if (!vendorDetails || vendorDetails.length === 0) return [];

        const productMap = new Map<string, ProductWiseData>();
    ...
    const calculateProductStats = (vendors: ProductWiseVendor[]): ProductStats => {
    ...
    const handleViewVendor = (vendorId: number): void => {
                                                                     ...
                                                                     <div className="mb-2" style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                                                        ...
                                                                        {/* PRODUCT SPECS */}
                                                                            <div className="d-flex flex-row gap-4">
                                                                                {product.specs && product.specs.reduce<ProductSpec[][]>((acc, spec, idx) => {

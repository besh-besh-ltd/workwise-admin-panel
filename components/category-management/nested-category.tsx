...
declare const bootstrap: {
  Collapse: {
    new (element: Element, options?: { toggle: boolean }): BootstrapCollapse;
    getInstance(element: Element | null): BootstrapCollapse | null;
  };
};
...
interface NestedCategoryProps {
  ...
  listLoading: boolean;
  handleUpdateCategory: (item: Category) => void;
  handleDeleteCat: (e: React.MouseEvent, item: Category) => void;
  handleAddCategory: (item: Category) => void;
}

const NestedCategory: React.FC<NestedCategoryProps> = (props) => {
  const {
    allCategories,
    listLoading,
  ...
  const toggleCollapse = (targetId: string): void => {
    const targetElement = document.getElementById(targetId);
    const bsCollapse = bootstrap.Collapse.getInstance(targetElement);
    bsCollapse?.toggle();
  };

  const renderAccordion = (items: Category[], parentAccordionId: string): React.ReactElement[] => {

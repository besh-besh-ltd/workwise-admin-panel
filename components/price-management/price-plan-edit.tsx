...
import { ErrorMessage, Field, FieldArray, Form, Formik, FormikHelpers, ArrayHelpers } from "formik";
import { useRouter } from "next/router";

interface FeatureItem {
	feature_id: number;
	allocated_feature: number | string;
}
...
const PlanEdit: React.FC = () => {
	const [featureLists, setFeatureLists] = useState<FeatureListItem[]>([]);
	...
	const getSubscriptionFeatureLists = (): void => {
		handleGetSubscriptionFeatureList(subscriptionDetails.user_type)
			.then((res: { data: FeatureListItem[] }) => {
	...
	const getSubscriptionDetails = (): void => {
		handleGetSubscriptionDetail(id)
			.then((res: { data: SubscriptionDataItem[] }) => {
	...
	const submitHandler = (values: SubscriptionDetails, resetForm: () => void): void => {
		values?.feature.forEach(obj => {
			obj.feature_id = String(obj.feature_id) as unknown as number;
	...
	const updateAWithFeatures = (features: FeatureListItem[]): void => {
    const featureMap: Record<number, number | string> = {};
			...
			<div className="card col-12">
				<div className="card-body mt-3">
					<Formik
						...
						onSubmit={(values: SubscriptionDetails, { resetForm }: FormikHelpers<SubscriptionDetails>) => {
							...
							<Form>
								<div className="form-group row">
									...
									<div className="col-sm-4">
										...
										<ErrorMessage
											name="name"
											render={(msg: string) => <div className="form-error">{msg}</div>}
										/>
									</div>
								</div>
								<div className="form-group row">
															...
															<input
																...
																onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
																	...
																	if (isChecked) {
																		...
																		const idx = (values.feature as unknown as number[]).indexOf(itemId);
														...
														<div className="col-sm-2">
															<Field
																...
																onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
																	...
																	const formikIndex = (values.feature as unknown as {id: number; allocated_feature: string}[]).findIndex(feature => feature.id === itemId);
																	if (formikIndex !== -1) {
																		arrayHelpers.replace(formikIndex, { ...(values.feature as unknown as {id: number; allocated_feature: string}[])[formikIndex], allocated_feature: newValue });
																	}
																}}
															/>
														</div>

...
interface ManipulatedFeature {
	feature_id: string;
	allocated_feature: string;
}

const PlanAdd: React.FC = () => {
	...
	const getSubscriptionFeatureLists = (): void => {
		handleGetSubscriptionFeatureList(selectedUserType)
	...
	const submitHandler = (values: FormValues, resetForm: () => void): void => {
		...
		const filteredFeature = values?.feature.filter(Boolean);
		const formData = {
			name: values.name,
			type: values.type,
			price: values.price,
		...
		const manipulatedFeature: ManipulatedFeature[] = formData?.feature.map(item => ({
			feature_id: (item.feature_id as unknown as string[])[0],
			allocated_feature: (item.allocated_feature ?? 0).toString()
		}));
                        ...
                        <Field
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                            ...
                            if (!checked) {
                              ...
                              setFieldValue(
                                `feature[${index}].allocated_feature`,
                                ""
                              );
                            } else {

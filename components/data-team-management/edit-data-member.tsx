...
const EditDataMemberPage: React.FC = () => {
    const router = useRouter();

    const [dataMemberData, setdataMemberData] = useState<DataMember[] | null>(null);
    ...
    const handledataMemberData = (): void => {
    ...
    const submitHandler = (values: FormValues, resetForm: () => void): void => {
        ...
        updateSubAdmin(updatedValues, router?.query?.id as string)
            .then((res : any) => {
                resetForm();
                toast(res.message);
                setTimeout(() => {
